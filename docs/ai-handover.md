...
| 19 | 选择列表里选了第三方管理器(如 MT「定位所在位置」), 日志显示「已调起」但手机毫无反应; 换系统「文件」却正常 | 默认目录候选 Intent 只设了 data/type, 没带 `FLAG_GRANT_READ_URI_PERMISSION`(自定义目录那条带了, 默认目录漏了): ExternalStorageProvider 的 content URI 必须随 Intent 派发临时读授权, 否则第三方应用拿到 URI 无权访问、Activity 静默退出; 系统 DocumentsUI 是特权组件不需要授权所以正常。`startActivity` 不抛异常 ≠ 对端能处理 | `buildDirIntents` 全部候选一律 `addFlags(FLAG_GRANT_READ_URI_PERMISSION)`。教训: **content URI 对外派发必带 grant flag,「调起成功」的日志不代表「打开成功」** |
