...
| 24 | 电脑睡眠唤醒后手机通知栏显示「已连接电脑（未连接）」，电脑→手机通、手机→电脑不通，电脑托盘只显示「安卓手机」无品牌 | `SyncForegroundService.onStartCommand()` 在睡眠唤醒后被再次调用，其内 `loadPreferences()` 无条件把 `currentPcIp` / `currentPcName` 重置为空 /「未连接」，但 `connectionState` 仍保持 1；outbound 同步只看 `currentPcIp` 于是跳过发送，心跳也因 IP 为空停止上报真实 `device_name`/`device_brand` | `loadPreferences()` 仅在 `connectionState != 1` 时才清空连接信息；`doBroadcastText()` 同时校验 `connectionState == 1` 与 `currentPcIp`；状态分裂不再发生。教训: **连接状态必须原子维护，不能一个字段已连接、另一个字段已断开** |
